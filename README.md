...
`npm install nolanium-wallet-backend --save`

Yarn:

`yarn add nolanium-wallet-backend`

## Documentation

[You can view the documentation here](https://turtlecoin.github.io/nolanium-wallet-backend/classes/_walletbackend_.walletbackend.html)

You can see a list of all the other classes on the right side of the screen.
Note that you will need to prefix them all with `WB.` to access them, if you are not using typescript style imports, assuming you imported with `const WB = require('nolanium-wallet-backend')`.

## Quick Start

You can find an [example project in the examples](https://github.com/NolaniumProject/nolanium-wallet-backend/tree/master/examples/example1) folder.

### Javascript

```javascript
const WB = require('nolanium-wallet-backend');
...
```typescript
import { WalletBackend, Daemon, IDaemon } from 'nolanium-wallet-backend';
...
`git clone https://github.com/NolaniumProject/nolanium-wallet-backend.git`

`cd nolanium-wallet-backend`
